    ...
    <nav className={`${styles.paddingX} w-full flex items-center py-5 fixed top-0 z-20 bg-primary`}>
      <div className='w-full flex justify-between items-center max-w-7xl mx-auto'>
        <Link to='/' className='flex items-center gap-2' onClick={() => {setActive(""); window.scrollTo(0,0)}}>
          ...
          <p className='text-white text-[18px] font-bold cursor-pointer flex'>Juan Schcolnik &nbsp;
        ...
        <div className='sm:hidden juystify-end  items-center'>
            ...
            <div className={`${!toggle ? "hidden" : "flex"} p-6 black-gradient absolut top-20 right-0 mx-4 my-2 min-w-[140px] z-10 rounded-xl`}>
              <ul className='list-none flex justify-end items-start flex-col gap-4'>
                {navLinks.map((link) => (
                  <li
                    key={link.id}
                    className={`${active === link.title ? "text-white" : "text-secondary"} font-poppins font-medium cursorpointer text-[16px]`}
